...
if(!($stmt = $conn->prepare("DELETE FROM coach WHERE coach.fname = ? AND coach.lname = ?"))){
	echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
}
if(!($stmt->bind_param("ss",$_POST['coach_first_Name'],$_POST['coach_last_Name']))){
	echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
}
if(!$stmt->execute()){
	echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
} else {
	echo "Removed " . $stmt->affected_rows . " row from coach.";
}
